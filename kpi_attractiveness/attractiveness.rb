class Attractiveness < Scout::Plugin
  ...
  def run
    ...
    data[:users_with_activity_(last_week)] = User.count(:conditions => ['user_logs.created_at > ?', Time.now - 1.week], :joins => 'LEFT JOIN user_logs ON user_logs.user_id = users.id', :group => 'users.id').size
